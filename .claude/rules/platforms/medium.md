...
- 命名規約: `yyyy-mm-<topic-kebab-case>`（Zenn と同一。例: `2026-08-slm-on-device`）
...
| `canonical` | Zenn や自社サイトからのクロスポスト時のみ。Import ツール経由なら自動設定される |
...
| `:::message` / `:::details` | Zenn 独自記法 | `:::message` → 太字の段落 + 引用ブロック。`:::details` → **展開して本文に置く**か、長大なら GitHub Gist へ逃がしてリンク |
...
- 数値は英語表記に統一: `30M parameters`, `38 MB`, `91.11%`。桁区切りはカンマ（`30,796,992`）だが、**地の文では丸めた値**を使い正確値は図表に集約する（Zenn と同じルール）
...
サムネイル（feature image）と差し込み画像は GPT（gpt-image 系）で生成する。**エージェントはプロンプトを設計し、`scripts/gen-image.sh`（Codex CLI 経由）で生成してローカルに保存するまでを担当する。生成結果の採否と Medium へのアップロードは人間が行う**。プロンプトは `05-medium.md` に記録する。

### 生成手順（エージェントが実行）

画像の生成は `scripts/gen-image.sh` で行う。Codex CLI の組み込み `image_gen` ツールを `codex exec` で非対話実行するラッパーで、**ChatGPT アカウントで `codex login` 済みなら動き、`OPENAI_API_KEY` は不要**（ChatGPT プランの Codex 利用枠を消費する）。この手順は note でも共用する。

```bash
scripts/gen-image.sh <prompt.txt> <out.png> [WIDTHxHEIGHT]   # 既定 1536x1024
```

1. プロンプトを `medium/images/<yyyy-mm>/<slug>-feature.prompt.txt` に保存する（`05-medium.md` に記録するものと同一内容。差し込み画像は `<slug>-fig1.prompt.txt` のように連番）
2. `scripts/gen-image.sh` を実行する。既存ファイルは上書きしないため、作り直す場合は `-v2` 等の別名にする
3. 生成された PNG を Read ツールで開き、**文字化け・画像内テキスト・矢印や関係の向きの逆転・他社ロゴの混入**がないか確認する。問題があればプロンプトを調整して再生成する（最大 3 回。それでも不可なら「手動生成待ち」として `05-medium.md` に記録する）
4. 生成結果（ファイルパス・サイズ・確認結果・試行回数）を `05-medium.md` の画像セクションに記録する

- サイズは各辺 16 の倍数・長辺 3840px 以下・比率 3:1 以内・総画素 655,360〜8,294,400 で指定する（Medium の feature image は `1536x1024` または `2048x1152`）。スクリプトは起動前にこの契約を検証する。`image_gen` にはサイズ引数がなく比率しか指示できないため、生成物の比率が要求と 5% 以内なら fit + 中央クロップ（`sips`）で正確なサイズに揃え、それ以上ずれていれば保存せず失敗にする（macOS 前提）
- スクリプトは Codex に `image_gen` 以外の能力を与えない: **シェルツールを無効化**（コマンド実行が不可能）、apps / plugins / hooks / MCP / Web 検索 / 子エージェントを無効化、`env -i` で環境変数を消去、一時 `CODEX_HOME`（`auth.json` のみ複製）で実行し、Codex のサンドボックスは read-only。さらに Codex の設定では止められない組み込みツール（`view_image` 等）による絶対パス読み取りを防ぐため、**codex プロセス全体を macOS の `sandbox-exec`（Seatbelt）で包み、ユーザー領域（`/Users`・`/Volumes`・`/Library`）と一時領域（`/private/tmp`・`/private/var/tmp`・`/private/var/folders`）配下のファイル読み取りを OS レベルで拒否する**（codex の配布ディレクトリとスクリプトの一時領域だけ許可。ホーム・`/private/tmp`・ユーザー一時ディレクトリに置いた画像を `view_image` で読めないこと、`image_gen` が動くことを検証済み）。プロンプトファイルは 8KB 上限・`<prompt>` タグ・制御文字を検査する。残る経路は「プロンプト文字列そのものが OpenAI に送られる」ことだけなので、**プロンプトに秘密情報を書かない**。また**プロンプトは信頼できない入力として扱い、本リポジトリのエージェントが本ワークフロー内で作成したものだけを渡す**（外部から受け取ったプロンプトファイルをそのまま渡さない）
- Codex CLI が未導入・未ログイン・実行失敗の場合、スクリプトは非ゼロ終了する。**エージェントは API キーの設定や代替ツールの導入を行わず**、「人間が ChatGPT で手動生成する」フォールバックとして `05-medium.md` に記録し Phase 5 を続行する（**画像ファイルの有無は Phase 5 の完了条件に含めない**。プロンプトが揃っていれば完了できる）
- **表・数値・文字を含む図には使わない**（下の画像仕様のとおり。作図ツール・スクリーンショットを人間が用意する）
- 生成した画像は補助的な成果物であり、人間が差し替え・破棄してよい
...
- 生成後はエージェントが Read ツールで確認し、人間が最終確認する: 文字化け・意味の逆転（矢印の向き等)・他社ロゴの混入がないか
...
- 表・Mermaid・`:::` 記法・脚注・H4 以下・ネストリストが**残っていない**こと（貼り付けで崩れる）
...
3. 「生成手順」に従い `scripts/gen-image.sh` で画像を生成して `medium/images/<yyyy-mm>/` に保存し、確認結果を `05-medium.md` に記録する（生成できない場合は「手動生成待ち」と記録して続行する）
4. 手動チェックリスト（上記）を通す

**公開（人間の担当範囲）** — Phase 5 完了後に、人間が任意のタイミングで実施する。

5. 生成済み画像を確認し、採用・差し替え・破棄を判断する（「手動生成待ち」の画像は ChatGPT で生成して `medium/images/<yyyy-mm>/` に保存する）
6. **人間が全文を読み、自分の言葉として責任を持てる内容か確認・加筆修正する**
7. 「公開モデル」の手順で Markdown をリッチテキスト化して Medium エディタに貼り付け、変換結果を目視確認のうえコードブロック・Pullquote・画像・タグを整える
8. Story settings で SEO title / description・タグを設定し（**クロスポストの場合は canonical に元記事の URL を設定する**。Import ツール経由なら自動設定される）、必要なら Publication に submit する
9. 「最重要の制約」の公式ポリシーを再確認し、開示文言の要否を判断したうえで公開する
...
代わりに Phase 5（Medium）で行うのは、**英語への変換・Medium 互換記法への変換・タイトル/サブタイトル/タグ設計・GPT 画像プロンプト作成と `scripts/gen-image.sh` による生成・公開チェックリスト**である。
