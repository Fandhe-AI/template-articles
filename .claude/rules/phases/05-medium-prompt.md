...
<instructions>
  ...
  **全文を利用してよい**。medium_competitor_analysis 等のリサーチ由来ブロックからは
  ...
  6. **手動チェックリストの実行**
     - 表・Mermaid・脚注・H4 以下・ネストリスト・単独行 URL が残っていないこと
     - リンク有効性、エンティティ表記、スペル・文法の通読確認

  7. **Phase 5 完了チェックリストの作成**
     - `status: draft` であること、上記チェック通過、画像プロンプト完備
     - **Phase 5 は `status: draft` のまま完了する。** 画像生成・Medium への貼り付け・
       Publication への submit・公開は、人間の作業として案内する（完了条件には含めない）
