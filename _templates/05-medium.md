...
生成は `scripts/gen-image.sh`（Codex CLI 経由）でエージェントが行い、採否と Medium へのアップロードは人間が行う。確認観点: **文字化け / 画像内テキスト / 矢印・関係の向き / 他社ロゴ混入**。生成できなかった場合は「手動生成待ち」と記録する（画像ファイルの有無は Phase 5 の完了条件に含めない）。

### 4.1 Feature image（サムネイル）

- 保存先: `medium/images/YYYY-MM/<slug>-feature.png`（プロンプト: 同 `<slug>-feature.prompt.txt`）
- 生成サイズ: `1536x1024` または `2048x1152`
- Alt text（英語）:
...
- [ ] `medium/images/YYYY-MM/` の生成画像を確認し、採用・差し替え・破棄を判断した（「手動生成待ち」は ChatGPT で生成して保存した）
